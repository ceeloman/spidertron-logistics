if not storage.logistic_spiders or not storage.providers or not storage.requesters then goto updated end

local removed = 0
for _, spider_data in pairs(storage.logistic_spiders) do
	removed = removed + 1
	spider_data[1].destroy()
end

storage.logistic_spiders = nil
storage.providers = nil
storage.requesters = nil
